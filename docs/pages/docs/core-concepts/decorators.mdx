...
To create a decorator, you can use the `createDecorator` function from the `psylon` package. This function takes a callback function as an argument, which will be executed when the decorator is applied to a service function.

```typescript
import {app, createDecorator} from 'psylon'
...
```typescript
import {app, createDecorator, ServiceError} from 'psylon'
...
```typescript
import {app, createDecorator, getContext} from 'psylon'
