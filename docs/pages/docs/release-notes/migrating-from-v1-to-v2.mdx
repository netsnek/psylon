...
You need to update your dependencies to the latest version of Pylon. The `psylon-cli` and `psylon-server` are no longer required and replaced by the `@netsnek/psylon-dev` package.
...
```typescript
import {defineService} from 'psylon'
